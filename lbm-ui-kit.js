...
(function (global, factory) {
    ...
    function loadBootstrapJS() {
        return new Promise((resolve, reject) => {
            ...
            script.src = '/v/1.0.0/assets/bootstrap-5.3.7/js/bootstrap.bundle.min.js';
    ...
    if (typeof document !== 'undefined') {
        loadBootstrapJS().catch(() => {
            // Bootstrap loading failed - components may not work properly
        });
    }
    ...
    class Modal {
        ...
        init() {
            ...
            this.backdrop = this.element.closest('.bs-modal') || this.element;

            // Read data attributes and update options
            const backdrop = this.backdrop.getAttribute('data-backdrop');
            const keyboard = this.backdrop.getAttribute('data-keyboard');
            const position = this.backdrop.getAttribute('data-position');
            
            if (backdrop !== null) {
                this.options.backdrop = backdrop !== 'false';
            }
            if (keyboard !== null) {
                this.options.keyboard = keyboard !== 'false';
            }
            if (position) {
            ...
            // Find close buttons
            this.closeButtons = this.element.querySelectorAll('[data-lbm-modal-close], .bs-modal-close');
        ...
        show() {
            ...
            const beforeShowEvent = new CustomEvent('bs.modal.beforeShow', {
                detail: { modal: this },
                cancelable: true
            });
            ...
            this.backdrop.classList.add('lbm-modal-open');

            // Focus management
            if (this.options.focus) {
                const modalBody = this.element.querySelector('.bs-modal-body');
                if (modalBody) {
            ...
            // Trigger show event
            const showEvent = new CustomEvent('bs.modal.show', {
                detail: { modal: this }
            });
        ...
        hide() {
            ...
            const beforeHideEvent = new CustomEvent('bs.modal.beforeHide', {
                detail: { modal: this },
                cancelable: true
            });
            ...
            // Remove open class
            this.backdrop.classList.remove('lbm-modal-open');
            ...
            const handleTransitionEnd = () => {
                ...
                const hiddenEvent = new CustomEvent('bs.modal.hidden', {
                    detail: { modal: this }
                });
            ...
            // Fallback with timeout management
    ...
    class Toast {
        ...
        static show(message, options = {}) {
            ...
            const toast = Utils.createElement('div', `lbm-toast lbm-toast-${config.type}`);

            // Create toast header with close button
            const toastHeader = Utils.createElement('div', 'lbm-toast-header');
            const headerTitle = Utils.createElement('div', 'lbm-toast-title', config.title);
            toastHeader.appendChild(headerTitle);

            // Add close button to header (top-right position)
            if (config.closable) {
                const closeBtn = Utils.createElement('button', 'lbm-toast-close', '×');
            ...
            // Create toast body
            const toastBody = Utils.createElement('div', 'lbm-toast-body');
            const toastMessage = Utils.createElement('div', 'lbm-toast-message');
            if (config.html) {
            ...
            const timestamp = Utils.createElement('div', 'lbm-toast-timestamp');
            const now = new Date();
            ...
            requestAnimationFrame(() => {
                toast.classList.add('lbm-toast-show');
            });
        ...
        static hide(toast) {
            toast.classList.remove('lbm-toast-show');
        ...
        static getContainer(position) {
            if (!Toast.containers[position]) {
                let container = document.querySelector(`.bs-toast-container.bs-${position}`);
                if (!container) {
                    container = Utils.createElement('div', `lbm-toast-container lbm-${position}`);
        ...
        static hideAll() {
            Object.values(Toast.containers).forEach(container => {
                const toasts = container.querySelectorAll('.bs-toast');
        ...
        static count() {
            let total = 0;
            Object.values(Toast.containers).forEach(container => {
                total += container.querySelectorAll('.bs-toast').length;
            });
            return total;
        }
    ...
    class Alert {
        ...
        init() {
            ...
            if (this.options.closable && !this.element.querySelector('.bs-alert-close')) {
                const closeBtn = Utils.createElement('button', 'lbm-alert-close', '×');
        ...
        close() {
            ...
            const beforeCloseEvent = new CustomEvent('bs.alert.beforeClose', {
                detail: { alert: this },
                cancelable: true
            });
            ...
            setTimeout(() => {
                ...
                const closedEvent = new CustomEvent('bs.alert.closed', {
                    detail: { alert: this }
                });
        ...
        static create(message, type = 'info', options = {}) {
            const alert = Utils.createElement('div', `lbm-alert lbm-alert-${type}`, message);

            const alertInstance = new Alert(alert, options);

            // Find container or create one
            let container = document.querySelector('.bs-alert-container');
            if (!container) {
                container = Utils.createElement('div', 'lbm-alert-container');
                document.body.appendChild(container);
            }
    ...
    class Confirm {
        static show(title, message, options = {}) {
            return new Promise((resolve) => {
                ...
                // Create overlay
                const overlay = Utils.createElement('div', 'lbm-modal lbm-modal-open');
                overlay.style.display = 'flex';

                // Create confirm dialog
                const confirmBox = Utils.createElement('div', 'lbm-modal-dialog lbm-modal-sm');
                confirmBox.innerHTML = `
                    <div class="lbm-modal-content">
                        <div class="lbm-modal-header">
                            <h3 class="lbm-modal-title">${title}</h3>
                        </div>
                        <div class="lbm-modal-body">
                            <p>${message}</p>
                        </div>
                        <div class="lbm-modal-footer">
                            <button class="lbm-btn lbm-btn-secondary" data-action="cancel">${config.cancelText}</button>
                            <button class="lbm-btn lbm-btn-${config.type}" data-action="confirm">${config.confirmText}</button>
                        </div>
                    </div>
                `;
    ...
    class Accordion {
        constructor(element) {
            this.element = element;
            this.items = Array.from(this.element.querySelectorAll('.bs-accordion-item'));
            this.init();
        }
        ...
        toggle(item) {
            const content = item.querySelector('.bs-accordion-content');
            if (item.classList.contains('lbm-open')) {
                item.classList.remove('lbm-open');
                content.style.maxHeight = null;
            } else {
                this.items.forEach(otherItem => {
                    if (otherItem !== item && otherItem.classList.contains('lbm-open')) {
                        otherItem.classList.remove('lbm-open');
                        otherItem.querySelector('.bs-accordion-content').style.maxHeight = null;
                    }
                });
                item.classList.add('lbm-open');
                content.style.maxHeight = content.scrollHeight + 'px';
            }
        }

        static initAll() {
            document.querySelectorAll('.bs-accordion').forEach(accordionElement => {
    ...
    class Carousel {
        constructor(element, options = {}) {
            ...
            this.slides = Array.from(this.element.querySelectorAll('.bs-carousel-slide'));
            this.currentIndex = 0;
            this.timer = null;
            this.init();
        }
        ...
        createControls() {
            const prevBtn = Utils.createElement('button', 'lbm-carousel-prev', '‹');
            const nextBtn = Utils.createElement('button', 'lbm-carousel-next', '›');
        ...
        updateSlides() {
            this.slides.forEach((slide, index) => {
                slide.classList.toggle('lbm-active', index === this.currentIndex);
            });
        }
    ...
    class SelectDropdown {
        ...
        createStructure() {
            ...
            this.container = Utils.createElement('div', 'lbm-select-dropdown');
            this.container.id = Utils.generateId();

            // Create trigger
            this.trigger = Utils.createElement('div', 'lbm-select-trigger');
            this.trigger.setAttribute('tabindex', '0');
            this.trigger.setAttribute('role', 'combobox');
            this.trigger.setAttribute('aria-expanded', 'false');
            this.trigger.setAttribute('aria-haspopup', 'listbox');

            this.valueDisplay = Utils.createElement('span', 'lbm-select-value lbm-placeholder');
            this.valueDisplay.textContent = this.options.placeholder;

            this.arrow = Utils.createElement('span', 'lbm-select-arrow');

            this.trigger.appendChild(this.valueDisplay);
            this.trigger.appendChild(this.arrow);

            // Create dropdown menu
            this.menu = Utils.createElement('div', 'lbm-select-dropdown-menu');
            this.menu.setAttribute('role', 'listbox');

            // Create search input if searchable
            if (this.options.searchable) {
                this.searchContainer = Utils.createElement('div', 'lbm-select-search');
                this.searchInput = Utils.createElement('input', 'lbm-select-search-input');
            ...
            this.optionsContainer = Utils.createElement('ul', 'lbm-select-options');
            ...
            if (this.options.size) {
                this.container.classList.add(`lbm-select-${this.options.size}`);
            }

            if (this.options.state) {
                this.container.classList.add(`lbm-select-${this.options.state}`);
            }

            if (this.options.disabled) {
                this.container.classList.add('lbm-select-disabled');
            }
        }
        ...
        renderOptions() {
            ...
            if (this.filteredOptions.length === 0) {
                const noResults = Utils.createElement('li', 'lbm-select-no-results');
        ...
        createOptionElement(option, index) {
            ...
            const button = Utils.createElement('button', 'lbm-select-option');
            button.type = 'button';
            ...
            if (option.disabled) {
                button.classList.add('lbm-disabled');
                button.disabled = true;
            }

            if (this.isSelected(option.value)) {
                button.classList.add('lbm-selected');
            ...
            if (option.icon) {
                content += `<span class="lbm-select-option-icon">${option.icon}</span>`;
            }

            content += `<div class="lbm-select-option-text">`;
            content += this.highlightSearchTerm(option.text);

            if (option.description) {
                content += `<div class="lbm-select-option-description">${this.highlightSearchTerm(option.description)}</div>`;
            }
        ...
        updateDisplay() {
            if (this.options.multiple) {
                if (this.selectedValues.length === 0) {
                    this.valueDisplay.textContent = this.options.placeholder;
                    this.valueDisplay.classList.add('lbm-placeholder');
                } else {
                    ...
                    this.valueDisplay.classList.remove('lbm-placeholder');
                }
            } else {
        ...
        open() {
            ...
            this.isOpen = true;
            this.container.classList.add('lbm-open');
            this.trigger.classList.add('lbm-open');
            this.trigger.setAttribute('aria-expanded', 'true');
            this.menu.classList.add('lbm-open');
        ...
        close() {
            ...
            this.isOpen = false;
            this.container.classList.remove('lbm-open');
            this.trigger.classList.remove('lbm-open');
            this.trigger.setAttribute('aria-expanded', 'false');
            this.menu.classList.remove('lbm-open');
        ...
        updateHighlight() {
            const options = this.optionsContainer.querySelectorAll('.bs-select-option');
            options.forEach((option, index) => {
                option.classList.toggle('lbm-highlighted', index === this.highlightedIndex);
            });
        }
        ...
        enable() {
            ...
            this.container.classList.remove('lbm-select-disabled');
        }
        ...
        loadFromJSON(jsonData, config = {}) {
            try {
                ...
                // Error loading JSON data - using empty array
        ...
        async loadFromAPI(url, config = {}) {
            try {
                ...
                // Error loading API data - using empty array
        ...
        showLoadingState(loadingText = 'Đang tải...') {
            ...
            if (this.container) {
                this.container.classList.add('lbm-select-loading');
            }
        }
        ...
        hideLoadingState() {
            if (this.container) {
                this.container.classList.remove('lbm-select-loading');
            }
        }
        ...
        showErrorState(errorMessage = 'Lỗi tải dữ liệu') {
            ...
            if (this.container) {
                this.container.classList.add('lbm-select-error');
            }
        }
        ...
        clearErrorState() {
            if (this.container) {
                this.container.classList.remove('lbm-select-error');
            }
        }

        // Static method to initialize all select dropdowns
        static initAll() {
            // Initialize dropdowns with data-lbm-select attribute (legacy)
            document.querySelectorAll('select[data-lbm-select]').forEach(select => {
            ...
            document.querySelectorAll('select.bs-select-dropdown').forEach(select => {
                if (!select._lbmSelectDropdown) {
                    const options = {};
    ...
    class InputDropdown {
        ...
        createStructure() {
            ...
            // Create dropdown container
            this.container = Utils.createElement('div', 'lbm-input-dropdown');
            this.container.id = Utils.generateId();

            // Create input field
            this.input = Utils.createElement('input', 'lbm-input-field');
            ...
            this.arrow = Utils.createElement('span', 'lbm-input-arrow');

            // Create input wrapper
            this.inputWrapper = Utils.createElement('div', 'lbm-input-wrapper');
            this.inputWrapper.appendChild(this.input);
            this.inputWrapper.appendChild(this.arrow);

            // Create dropdown menu
            this.menu = Utils.createElement('div', 'lbm-input-dropdown-menu');
            this.menu.setAttribute('role', 'listbox');

            // Create options list
            this.optionsList = Utils.createElement('div', 'lbm-input-options-list');
        ...
        parseOptions() {
            ...
            if (this.element.dataset.options) {
                try {
                    ...
                    // Invalid JSON in data-options attribute - using empty object
                }
            }
        ...
        handleOptionClick(e) {
            const optionElement = e.target.closest('.bs-input-option');
            if (optionElement) {
        ...
        renderOptions() {
            ...
            if (this.filteredOptions.length === 0) {
                const noResults = Utils.createElement('div', 'lbm-input-no-results');
                noResults.textContent = this.options.noResultsText;
            ...
            this.filteredOptions.forEach((option, index) => {
                const optionElement = Utils.createElement('div', 'lbm-input-option');
                optionElement.dataset.index = index;
                optionElement.textContent = option.text;

                if (option.disabled) {
                    optionElement.classList.add('lbm-disabled');
                }
        ...
        updateHighlight() {
            const options = this.optionsList.querySelectorAll('.bs-input-option');
            options.forEach((option, index) => {
                option.classList.toggle('lbm-highlighted', index === this.highlightedIndex);
            });
        }
        ...
        positionDropdown() {
            ...
            if (spaceBelow < this.options.maxHeight && spaceAbove > spaceBelow) {
                this.menu.classList.add('lbm-dropdown-up');
            } else {
                this.menu.classList.remove('lbm-dropdown-up');
            }
        }
        ...
        enable() {
            this.options.disabled = false;
            this.container.classList.remove('lbm-input-disabled');
            this.input.disabled = false;
        }

        disable() {
            this.options.disabled = true;
            this.container.classList.add('lbm-input-disabled');
        ...
        showLoadingState(text = this.options.loadingText) {
            this.optionsList.innerHTML = `<div class="lbm-input-loading">${text}</div>`;
            this.container.classList.add('lbm-loading');
        }

        hideLoadingState() {
            this.container.classList.remove('lbm-loading');
        }

        showErrorState(message) {
            this.optionsList.innerHTML = `<div class="lbm-input-error">Lỗi: ${message}</div>`;
            this.container.classList.add('lbm-error');
        }

        // Clear error state
        clearErrorState() {
            this.container.classList.remove('lbm-error');
        }
        ...
        static initAll() {
            // Initialize dropdowns with lbm-input-dropdown class (support Bootstrap form-control)
            document.querySelectorAll('input[class*="lbm-input-dropdown"]').forEach(input => {
    ...
    class ThemeSwitcher {
        ...
        bindEvents() {
            document.addEventListener('click', (e) => {
                if (e.target.matches('[data-lbm-theme-toggle]')) {
                    e.preventDefault();
                    this.toggle();
                }
            });
        ...
        applyTheme(theme) {
            ...
            document.querySelectorAll('[data-lbm-theme-toggle]').forEach(btn => {
    ...
    const LBM = {
        ...
        init: function () {
            ...
            // Initialize modals
            document.querySelectorAll('.bs-modal').forEach(modalElement => {
                const modalDialog = modalElement.querySelector('.bs-modal-dialog');
                if (modalDialog && !modalDialog._lbmModal) {
                    modalDialog._lbmModal = new Modal(modalDialog);
                }
            });

            // Initialize alerts
            document.querySelectorAll('.bs-alert').forEach(alertElement => {
                if (!alertElement._lbmAlert) {
                    alertElement._lbmAlert = new Alert(alertElement);
                }
            });

            // Initialize carousels
            document.querySelectorAll('.bs-carousel').forEach(carouselElement => {
                if (!carouselElement._lbmCarousel) {
                    carouselElement._lbmCarousel = new Carousel(carouselElement);
                }
            });

            // Modal trigger buttons - only add once
            if (!this._modalClickHandler) {
                this._modalClickHandler = (e) => {
                    if (e.target.matches('[data-lbm-modal]')) {
                        e.preventDefault();
                        
                        const target = e.target.getAttribute('data-lbm-modal');
                        const buttonId = e.target.id || e.target.getAttribute('data-lbm-modal');
                        
                        // Debounce rapid clicks on same button
                        if (this._modalClickDebounce.has(buttonId)) {
                            return;
                        }
                        
                        this._modalClickDebounce.set(buttonId, true);
                        setTimeout(() => {
                            this._modalClickDebounce.delete(buttonId);
                        }, 200);
                        
                        const modal = document.querySelector(target);
                        if (modal) {
                            const modalDialog = modal.querySelector('.bs-modal-dialog');
                            if (modalDialog && modalDialog._lbmModal) {
                ...
                document.addEventListener('click', this._modalClickHandler);
            }

            // LBM UI Kit initialized successfully
        }
    };
